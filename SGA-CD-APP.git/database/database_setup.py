...
def setup_database_with_conn(conn: sqlite3.Connection):
    """
    Crea todas las tablas necesarias para la aplicación en una conexión de base de datos existente.
    Esta función es reutilizable para crear tanto la base de datos central como las locales.
    """
    cursor = conn.cursor()

    # --- 0. Tabla de Inquilinos (Tenants) ---
    # Esta tabla solo será relevante en la base de datos central.
    # En las bases de datos locales, estará mayormente vacía.
    cursor.execute("""
    ...
    CREATE TABLE IF NOT EXISTS usuarios (
        ...
        rol TEXT NOT NULL CHECK(rol IN (
            'admin_empresa', 'admin_general', 'jefe_area', 'coordinador',
            'profesor', 'alumno', 'almacenista', 'jefe_almacen', 'jefe_escenarios',
            'profesional', 'tecnico', 'asistencial'
        )),
        nombre_completo TEXT,
        correo TEXT,
        reporta_a_usuario_id INTEGER,
        reset_token TEXT,
    ...
    # --- 2. Tablas Específicas por Rol y Tabla de Enlace de Áreas ---
    ...
    CREATE TABLE IF NOT EXISTS jefes_area (
        ...
        usuario_id INTEGER UNIQUE NOT NULL,
        inquilino_id INTEGER NOT NULL,
        area_responsabilidad TEXT CHECK(area_responsabilidad IN ('Cultura', 'Deportes')),
        FOREIGN KEY (usuario_id) REFERENCES usuarios(id),
        FOREIGN KEY (inquilino_id) REFERENCES inquilinos(id)
    );
    ...
    CREATE TABLE IF NOT EXISTS eventos (
        ...
        nombre TEXT NOT NULL,
        descripcion TEXT,
        tipo TEXT NOT NULL CHECK(tipo IN ('evento', 'salida')),
    ...
    """)
    # --- Columnas añadidas dinámicamente para evitar errores en DBs existentes ---
    try:
    ...
    try:
        ...
        pass # La columna ya existe
    try:
        cursor.execute("ALTER TABLE alumnos ADD COLUMN mostrar_en_rankings INTEGER DEFAULT 1")
    except sqlite3.OperationalError:
        pass # La columna ya existe
    try:
    ...
    conn.commit()
    print("Base de datos y tablas actualizadas.")


def setup_database():
    """
    Crea y/o conecta a la base de datos principal 'formacion.db' y configura el esquema.
    Esta función está pensada para ser usada por el servidor API central.
    """
